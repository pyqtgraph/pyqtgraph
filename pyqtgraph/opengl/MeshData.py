...
class MeshData(object):
    ...
    def setVertexColors(self, colors, indexed=None):
        ...
        if indexed is None:
            correct_shape = (len(self._vertexes), 4)
            if colors.shape != correct_shape:
                raise ValueError('colors.shape (%s) must be %s'
                                 % (colors.shape, correct_shape))
    ...
    def setFaceColors(self, colors, indexed=None):
        ...
        colors = np.asanyarray(colors)
        if indexed is None:
            correct_shape = (len(self._faces), 4)
            if colors.shape != correct_shape:
                raise ValueError('colors.shape (%s) must be %s'
                                 % (colors.shape, correct_shape))
