...
ArrayType = Union[MetaArray, npt.ArrayLike]


def downsample(data: ArrayType, n: int, axis: int = 0, xvals: str = 'subsample') -> ArrayType:
...
def applyFilter(data: MetaArray, b: npt.ArrayLike, a: npt.ArrayLike, padding: int = 100,
                bidir: bool = True) -> MetaArray:
...
def besselFilter(data: MetaArray, cutoff: float, order: int = 1, dt: Optional[float] = None, btype: str = 'low',
                 bidir: bool = True) -> MetaArray:
...
def butterworthFilter(data: MetaArray, wPass: float, wStop: Optional[float] = None, gPass: float = 2.0,
                      gStop: float = 20.0, order: int = 1, dt: Optional[float] = None, btype: str = 'low',
                      bidir=True) -> MetaArray:
    """return data passed through bessel filter"""
    try:
        import scipy.signal
    except ImportError:
...
def rollingSum(data: ArrayType, n: int) -> np.ndarray:
    d1 = data.copy()
    d1[1:] += d1[:-1]  # integrate
...
def mode(data: npt.ArrayLike, bins: Optional[int] = None) -> np.ndarray:
...
def modeFilter(data: np.ndarray, window: int = 500, step: Optional[int] = None,
               bins: Optional[int] = None) -> ArrayType:
...
def denoise(data: np.ndarray, radius: int = 2, threshold: int = 4) -> np.ndarray:
    ...
    d5 = np.where(mask, d1[:-r2],
                  d1[radius:-radius])  # where both are true replace the value with the value from 2 points before
...
def adaptiveDetrend(data: MetaArray, x=None, threshold: float = 3.0) -> MetaArray:
...
def histogramDetrend(data: np.ndarray, window: int = 500, bins: int = 50, threshold: float = 3.0,
                     offsetOnly: bool = False) -> np.ndarray:
...
def concatenateColumns(data: ArrayType) -> np.ndarray:
...
def removePeriodic(data: ArrayType, f0: float = 60.0, dt: Optional[float] = None, harmonics: int = 10,
                   samples: int = 4) -> ArrayType:
    if (hasattr(data, 'implements') and data.implements('MetaArray')):
        data1 = data.asarray()
        if dt is None:
    ...
    for i in range(1, harmonics + 2):
        ...
        for j in range(ind1, ind2 + 1):
            phase = np.angle(ft[
                                 j])  ## Must preserve the phase of each point, otherwise any transients in the trace might lead to large artifacts.
            re = mag * np.cos(phase)
            im = mag * np.sin(phase)
            ft[j] = re + im * 1j
