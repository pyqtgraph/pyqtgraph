...
def downsample(data: npt.ArrayLike, n: int, axis: int = 0, xvals: str = 'subsample') -> npt.ArrayLike:
...
def applyFilter(data: npt.ArrayLike, b: npt.ArrayLike, a: npt.ArrayLike, padding: int = 100,
                bidir: bool = True) -> npt.ArrayLike:
...
def rollingSum(data: npt.ArrayLike, n: int) ->npt.ArrayLike:
    d1 = data.copy()
    d1[1:] += d1[:-1]  # integrate
...
def mode(data: npt.ArrayLike, bins: Optional[int] = None) -> npt.ArrayLike:
...
def modeFilter(data: npt.ArrayLike, window: int = 500, step: Optional[int] = None,
               bins: Optional[int] = None) -> npt.ArrayLike:
...
def denoise(data: npt.ArrayLike, radius: int = 2, threshold: int = 4) -> npt.ArrayLike:
...
def adaptiveDetrend(data: npt.ArrayLike, x=None, threshold: float = 3.0) -> npt.ArrayLike:
...
def histogramDetrend(data: npt.ArrayLike, window: int = 500, bins: int = 50, threshold: float = 3.0,
                     offsetOnly: bool = False) -> npt.ArrayLike:
...
def concatenateColumns(data: npt.ArrayLike) -> npt.ArrayLike:
...
def removePeriodic(data: npt.ArrayLike, f0: float = 60.0, dt: Optional[float] = None, harmonics: int = 10, samples: int=4) -> npt.ArrayLike:
    data1 = data
    if dt is None:
