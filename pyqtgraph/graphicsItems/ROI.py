...
class ROI(GraphicsObject):
    ...
    **Signals**
    ...
    def setPos(self, pos, update=True, finish=True):
        ...
        By default, this will cause both sigRegionChanged and
        sigRegionChangeFinished to be emitted.

        If finish is False, then sigRegionChangeFinished will not be
        emitted. You can then use stateChangeFinished() to cause the signal to
        be emitted after a series of state changes.

        If update is False, the state change will be remembered but
        not processed and no signals will be emitted. You can then use
        stateChanged() to complete the state change. This allows multiple
        change functions to be called sequentially while minimizing processing
        overhead and repeated signals. Setting update=False also forces
        finish=False.
        """
    ...
    def setSize(self, size, update=True, finish=True):
        """Set the size of the ROI. May be specified as a QPoint, Point, or
        list of two values.
    ...
    def translate(self, *args, **kargs):
        ...
        *snap* can be:
        ...
        ============== =========================================================
        None (default) use self.translateSnap and self.snapSize to determine
                       whether/how to snap
        False          do not snap
    ...
    def addFreeHandle(self,
        ...
        **Arguments**
        ...
        if pos is not None:
            pos = Point(pos)
        return self.addHandle(
            {'name': name, 'type': 'f', 'pos': pos, 'item': item}, index=index)
    ...
    def addRotateFreeHandle(self, pos, center,
        ...
        **Arguments**
        ...
        center = Point(center)
        return self.addHandle(
            {'name': name, 'type': 'rf', 'center': center, 'pos': pos, 'item': item},
            index=index)

    def addHandle(self, info, index=None):
        ## If a Handle was not supplied, create it now
        if 'item' not in info or info['item'] is None:
            h = Handle(self.handleSize,
                       typ=info['type'], pen=self.handlePen, parent=self)
    ...
    def indexOfHandle(self, handle):
        ...
        if isinstance(handle, Handle):
            index = [i for i, info in enumerate(self.handles)
                     if info['item'] is handle]
            if len(index) == 0:
                raise Exception(
                    "Cannot remove handle; it is not attached to this ROI")
            return index[0]
        else:
            return handle
    ...
    def getSceneHandlePositions(self, index=None):
        ...
        if index == None:
            ...
            return (self.handles[index]['name'],
                    self.handles[index]['item'].scenePos())
    ...
    def hoverEvent(self, ev):
        ...
        if not ev.isExit():
            ...
            for btn in [QtCore.Qt.LeftButton,
                        QtCore.Qt.RightButton,
                        QtCore.Qt.MidButton]:
                if (int(self.acceptedMouseButtons() & btn) > 0
                    and ev.acceptClicks(btn)):
                    hover=True
    ...
    def mouseDragEvent(self, ev):
        if ev.isStart():
            ...
            if ev.button() == QtCore.Qt.LeftButton:
                ...
                if self.translatable:
                    ...
                    self.preMoveState = self.getState()
                    self.cursorOffset = (
                        self.pos() - self.mapToParent(ev.buttonDownPos()))
        ...
        if (self.translatable and self.isMoving and
            ev.buttons() == QtCore.Qt.LeftButton):
            snap = bool(ev.modifiers() & QtCore.Qt.ControlModifier)
    ...
    def stateChanged(self, finish=True):
        ...
        If there are any changes, then the positions of handles are updated
        accordingly and sigRegionChanged is emitted. If finish is True, then
    ...
    def stateRect(self, state):
        ...
        r = tr.mapRect(r)
        return r.adjusted(
            state['pos'][0], state['pos'][1], state['pos'][0], state['pos'][1])
    ...
    def boundingRect(self):
        return QtCore.QRectF(
            0, 0, self.state['size'][0], self.state['size'][1]).normalized()
    ...
    def getArraySlice(self, data, img, axes=(0,1), returnSlice=True):
        ...
        from data covered by this ROI.  Also returns the transform which maps
        the ROI into data coordinates.

        If returnSlice is set to False, the function returns a pair of tuples
        with the values that would have been used to generate the slice
        objects ((ax0Start, ax0Stop), (ax1Start, ax1Stop)).

        If the slice can not be computed (usually because the scene/transforms
        are not properly constructed yet), then the method returns None.
        """
        #print "getArraySlice"
        ...
        bounds = (
            (int(min(intBounds.left(), intBounds.right())),
    ...
    def getArrayRegion(self, data, img, axes=(0,1), returnMappedCoords=False,
        ...
        **Arguments**
        ...
        This method uses :func:`affineSlice <pyqtgraph.affineSlice>` to
        generate the slice from *data* and uses :func:`getAffineSliceParams
        <pyqtgraph.ROI.getAffineSliceParams>` to determine the parameters to
        pass to :func:`affineSlice <pyqtgraph.affineSlice>`.

        If *returnMappedCoords* is True, then the method returns a tuple
        (result, coords) such that coords is the set of coordinates used to
        interpolate values from the original data, mapped into the parent
        coordinate system of the image. This is useful, when slicing data from
        images that have been transformed, for determining the location of each
        value in the sliced data.
        ...
        if not returnMappedCoords:
            return fn.affineSlice(
                data, shape=shape, vectors=vectors, origin=origin, axes=axes,
                **kwds)
        else:
            kwds['returnCoords'] = True
            result, coords = fn.affineSlice(
                data, shape=shape, vectors=vectors, origin=origin, axes=axes,
                **kwds)
    ...
    def getAffineSliceParams(self, data, img, axes=(0,1)):
        ...
        <pyqtgraph.affineSlice>` to extract a subset of *data* using this ROI
    ...
    def getGlobalTransform(self, relativeTo=None):
        """Return global transformation (rotation angle+translation) required
        to move from relative state to current state. If relative state isn't
        specified, then we use the state of the ROI when mouse is pressed.
        """
        if relativeTo == None:
...
class Handle(UIGraphicsItem):
    ...
    def __init__(self, radius,
                 typ=None, pen=(200, 200, 220), parent=None, deletable=False):
    ...
    def setDeletable(self, b):
        ...
        if b:
            self.setAcceptedMouseButtons(
                self.acceptedMouseButtons() | QtCore.Qt.RightButton)
        else:
            self.setAcceptedMouseButtons(
                self.acceptedMouseButtons() & ~QtCore.Qt.RightButton)
...
class MultiLineROI(MultiRectROI):
    def __init__(self, *args, **kwds):
        ...
        print("Warning: MultiLineROI has been renamed to MultiRectROI. "
              "(and MultiLineROI may be redefined in the future)")
...
class CircleROI(EllipseROI):
    ...
    **Arguments**
    ...
    def __init__(self, pos, size, **args):
        ...
        #self.addTranslateHandle([0.5, 0.5])
        self.addScaleHandle(
            [0.5*2.**-0.5 + 0.5, 0.5*2.**-0.5 + 0.5], [0.5, 0.5])
...
class PolygonROI(ROI):
    ...
    def boundingRect(self):
        ...
        for h in self.handles:
            r |= self.mapFromItem(
                h['item'], h['item'].boundingRect()).boundingRect()   ## |= gives the union of the two QRectFs
        return r
...
class PolyLineROI(ROI):
    ...
    **Arguments**
    ...
    def addSegment(self, h1, h2, index=None):
        seg = LineSegmentROI(
            handles=(h1, h2), pen=self.pen, parent=self, movable=False)
        if index is None:
    ...
    **Arguments**
    ...
    def getArrayRegion(self, data, img, axes=(0,1)):
        ...
        for i in range(len(imgPts)-1):
            ...
            o = Point(imgPts[i])
            r = fn.affineSlice(
                data, shape=(int(d.length()),), vectors=[Point(d.norm())],
                origin=o, axes=axes, order=1)
            rgns.append(r)
...
class SpiralROI(ROI):
    ...
    def stateChanged(self, finish=True):
        ...
        if len(self.handles) > 1:
            ...
            self.path.moveTo(
                QtCore.QPointF(a*theta*cos(theta), a*theta*sin(theta)))
