    ...
    def _updateDataRect(self):
        ...
        if self.containsNonfinite is False:  # all points are directly usable.
            ...
            selection = np.isfinite(
                self.y)  # We are looking for the bounds of the plottable data set. Infinite and Nan are ignored.
            ...
            try:
                ...
                ymax = np.nan
            selection = np.isfinite(
                self.x)  # We are looking for the bounds of the plottable data set. Infinite and Nan are ignored.
            ...
            try:
                ...
                xmin = np.nan
                xmax = np.nan
            self.containsNonfinite = not (
                        all_x_are_finite and all_y_are_finite)  # This always yields a definite True/False answer
    ...
    **Signals:**
    ...
    def updateItems(self, styleUpdate=True):
        ...
        if (
            ...
            if isinstance(curveArgs['connect'], str) and curveArgs[
                'connect'] == 'auto':  # connect can also take a boolean array
                if dataset.containsNonfinite is None:
                    curveArgs[
                        'connect'] = 'all'  # this is faster, but silently connects the curve across any non-finite values
                else:
