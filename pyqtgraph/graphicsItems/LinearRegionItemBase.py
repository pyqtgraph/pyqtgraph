...
__all__ = ['LinearRegionItemBase']

class LinearRegionItemBase(UIGraphicsItem):
    """
    ...
    **Signals:**
    ...
    def __init__(self, values=[0,1], orientation=None, brush=None, movable=True, bounds=None):
        """Create a new LinearRegionItemBase.
        
        ==============  =====================================================================
        **Arguments:**
        values          A list of the positions of the lines in the region. These are not
                        limits; limits can be set by specifying bounds.
        orientation     Options are LinearRegionItemBase.Vertical or LinearRegionItemBase.Horizontal.
        ...
        if orientation is None:
            orientation = LinearRegionItemBase.Vertical
        ...
        if orientation == LinearRegionItemBase.Horizontal:
            self.lines = [
                InfiniteLineBase(QtCore.QPointF(0, values[0]), 0, movable=movable, bounds=bounds), 
                InfiniteLineBase(QtCore.QPointF(0, values[1]), 0, movable=movable, bounds=bounds)]
        elif orientation == LinearRegionItemBase.Vertical:
            self.lines = [
                InfiniteLineBase(QtCore.QPointF(values[1], 0), 90, movable=movable, bounds=bounds), 
                InfiniteLineBase(QtCore.QPointF(values[0], 0), 90, movable=movable, bounds=bounds)]
        else:
    ...
    def setBounds(self, bounds):
        ...
        See :func:`setRegion <pyqtgraph.LinearRegionItemBase.setRegion>` to set the position 
        of the region."""
        for l in self.lines:
            l.setBounds(bounds)
    ...
    def boundingRect(self):
        ...
        rng = self.getRegion()
        if self.orientation == LinearRegionItemBase.Vertical:
            br.setLeft(rng[0])
            br.setRight(rng[1])
        else:
            br.setTop(rng[0])
