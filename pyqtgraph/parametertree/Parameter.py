...
class Parameter(QtCore.QObject):
    ...
    **Signals:**
    ...
    def get_basic_options_from_xml_element(el: Element):
        """
        Extract basic options from an XML element and return them as a dictionary.

        This function retrieves attributes such as `type`, `title`, `visible`, and other
        configuration flags from an XML element and organizes them into a dictionary.

        Args:
            el (xml.etree.ElementTree.Element): The XML element containing the options as attributes.

        Returns:
            dict: A dictionary containing the following all the options extracted from the XML element.
        """
        basic_options = {
            "name": el.tag,
            "type": el.get('type'),
    ...
    def get_basics_options_from_parameter(param: 'Parameter'):
        """
        Extract basic options from a Parameter object and return them as a dictionary.

        This function retrieves options such as `type`, `title`, visibility, and other
        boolean or additional settings from a Parameter object. Boolean flags are converted 
        into a string format ('1' for True, '0' for False).

        Args:
            param (pyqtgraph.parametertree.Parameter): The Parameter object containing the options.

        Returns:
            dict: A dictionary containing all the options extracted from the Parameter object.
        """
        opts = {
