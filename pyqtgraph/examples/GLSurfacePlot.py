...
cols = 90
rows = 100
x = np.linspace(-8, 8, cols+1, dtype=np.float32).reshape((-1,1))
y = np.linspace(-8, 8, rows+1, dtype=np.float32).reshape((1,-1))
d = (x**2 + y**2) * 0.1
d2 = d ** 0.5 + 0.1
...
p4.shader()['colorMap'] = [0.2, 2, 0.5, 0.2, 1, 1, 0.2, 0, 2]
