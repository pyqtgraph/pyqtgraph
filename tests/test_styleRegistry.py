...
def test_specification_formats():
    ...
    for expectation, desc in args: # test pen registration
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore") # getRegisteredPen raises an expected warning for a test with unknown identifier
            pen = reg.getRegisteredPen(desc)
        if expectation is None:
            # print( desc, pen )
            assert pen is None # does not match input patterns
        elif expectation: # matches input pattern for registered pen
    ...
    args = ( # Do we expect a registered brush (or 'None'?) for this parameter?
        ( True, ('p0', 255) ),
        ( True, 'p1'),
        ( True, ('p2', 0.1) ),
        ( False, ('#00112233') ),
