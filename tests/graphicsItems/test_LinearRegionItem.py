...
def test_clip_to_plot_data_item(orientation):
    ...
    lr = pg.LinearRegionItem(init_bounds, clipItem=pdi, orientation=orientation)
    p.addItem(lr)
...
def test_disable_clip_item():
    ...
    lr = pg.LinearRegionItem(init_bounds, clipItem=img)
    p.addItem(lr)

    app.processEvents()

    # initial bound that was out of range snaps to the imageitem bound
    check_region(lr, (init_bounds[0], img.height()), exact=True)

    # disable clipItem, move the right InfiniteLine beyond the new bound
    lr.setBounds(init_bounds)
    lr.lines[1].setPos(init_bounds[1] + 10)

    app.processEvents()

    check_region(lr, init_bounds, exact=True)
...
def test_clip_to_item_in_other_vb():
    ...
    lr = pg.LinearRegionItem(init_bounds)
    p2.addItem(lr)
    app.processEvents()
    ...
    # snap to image width
    check_region(lr, (init_bounds[0], img_shape[1]), exact=True)
